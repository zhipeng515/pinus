    ...
    export class WSClient{
        ...
        private heartbeat(data){
            ...
            self.heartbeatId = setTimeout(function() {
                ...
                self.heartbeatTimeoutId = setTimeout(self.heartbeatTimeoutCb.bind(self,data), self, self.heartbeatTimeout);
            }, self, self.heartbeatInterval);
        }
        private heartbeatTimeoutCb(data) {
            var gap = this.nextHeartbeatTimeout - Date.now();
            if(gap > this.gapThreshold) {
                this.heartbeatTimeoutId = setTimeout(this.heartbeatTimeoutCb.bind(this,data), this, gap);
            } else {
